    ...
    <section class="bg-white border-2 border-black rounded-[2.5rem] p-6 md:p-8 shadow-sm">
        ...
        <form action="/admin/etudiants/update" method="POST" id="profileForm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="admin-card bg-white p-6">
                    <label class="block text-xs font-black uppercase text-gray-400 mb-2">Password</label>
                    <input
                            value="••••••••••••"
                            type="password"
                            class="w-full bg-gray-50 border-2 border-black rounded-2xl px-4 py-3 font-bold outline-none"
                    ...
                    />
                    <p class="text-[10px] text-gray-400 font-semibold italic mt-2">
                        Password is hidden and locked.
                    </p>
                </div>

                <!-- ROLE (editable when Edit) -->
...
<script>
    ...
    const editable = Array.from(document.querySelectorAll('[data-can-edit="true"]'));
    ...
    function setEditing(state) {
        ...
        // Toggle readonly/disabled ONLY for editable fields
        editable.forEach((el) => {
        ...
        if (isEditing) {
            ...
            // IMPORTANT: when editing, clicking button should SUBMIT the form
            ...
            // When locked, clicking should NOT submit, it should toggle to edit
    ...
    editBtn?.addEventListener("click", (e) => {
        // If locked -> unlock (do not submit)
        if (!isEditing) {
            e.preventDefault();
            setEditing(true);
            return;
        ...
        // If editing and button is submit: allow submit.
        // But before submit, lock UI back immediately for UX (optional)
        setEditing(false);
        // Form will submit because type="submit"
    });

    // Start locked
    setEditing(false);

    // Optional: update header title when name changes (nice UX)
    document.getElementById("nameInput")?.addEventListener("input", (e) => {
        const t = document.getElementById("studentTitle");
