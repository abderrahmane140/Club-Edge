    ...
    <aside class="w-64 bg-black text-white p-8 flex flex-col hidden lg:flex">
        ...
        <nav class="flex-grow space-y-4">
            ...
            <a href="admin/create" class="flex items-center gap-3 text-gray-400 hover:text-white sidebar-item transition">
...
<tr class="border-b border-gray-50 hover:bg-gray-50 transition cursor-pointer"
    data-href="/admin/etudiants/{{ $user->id }}">

    <td class="py-4 px-4">
        <p class="font-extrabold">{{ $user->name }}</p>
        <p class="text-[10px] text-gray-500 italic">{{ $user->role }}</p>
    </td>
    ...
    <td class="py-4 px-4 text-right">
        <form action="/admin/delete" method="POST" class="inline" data-no-rowclick>
            <input type="hidden" name="std_id" value="{{ $user->id }}" />
            <button type="button"
                class="delete-btn px-4 py-2 rounded-full border-2 border-black bg-red-600 text-white font-black text-xs">
...
@endforeach
    ...
    <!-- DELETE MODAL -->
    <div id="deleteModal" class="fixed inset-0 bg-black/80 hidden z-50 flex items-center justify-center backdrop-blur-sm opacity-0 transition-opacity duration-300">
        <div class="bg-white border-4 border-black rounded-[2rem] p-8 max-w-md w-full mx-4 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] transform scale-90 transition-transform duration-300" id="deleteModalContent">
            <div class="text-center">
                <div class="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4 border-2 border-black">
                    <i class="fa-solid fa-triangle-exclamation text-2xl text-red-600"></i>
                </div>
                <h3 class="text-2xl font-black uppercase mb-2">Attention !</h3>
                <p class="text-gray-600 font-medium mb-8">Voulez-vous vraiment supprimer cet étudiant ? Cette action est irréversible.</p>
                
                <div class="flex gap-4 justify-center">
                    <button id="cancelDelete" class="px-6 py-3 rounded-full border-2 border-black font-bold hover:bg-gray-100 transition w-full">
                        Annuler
                    </button>
                    <button id="confirmDelete" class="px-6 py-3 rounded-full border-2 border-black bg-red-600 text-white font-bold hover:bg-red-700 transition w-full shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                        Supprimer
                    </button>
                </div>
            </div>
